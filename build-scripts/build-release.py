...
"""
This script is shared between SDL3, SDL3_image, SDL3_mixer, SDL3_net and SDL3_ttf.
...
class Releaser:
    ...
    def add_files_recursively(self, archived_files: dict[str, Path], arcdirs: list[str], paths: list[Path]):
        logger.debug(f"add_files_recursively({arcdirs=} {paths=})")
        for path in paths:
            for arcdir in arcdirs:
                arcname = f"{arcdir}{path.name}"
                if path.is_file():
                    logger.debug("Adding %s as %s", path, arcname)
                    archived_files[arcname] = path
                elif path.is_dir():
                    self.add_files_recursively(archived_files=archived_files, arcdirs=[f"{arcdir}{path.name}/"], paths=list(path.iterdir()))
                else:
    ...
    def create_mingw_archives(self) -> None:
            ...
            for dep in self.release_info["dependencies"].keys():
                ...
                with chdir(extract_dir):
                    ...
                    for arch, triplet in ARCH_TO_TRIPLET.items():
                        #FIXME: install-all (SDL3) vs install-package (SDL2) -> parametrize
                        self.executer.run(["make", f"-C", str(extract_dir), f"-j{os.cpu_count()}", ARCH_TO_INSTALL_TARGET[arch], f"DESTDIR={str(mingw_deps_path / triplet)}"])
        ...
        if "cmake" in self.release_info["mingw"]:
            assert self.release_info["mingw"]["cmake"]["shared-static"] in ("args", "build_shared_libs")
            for arch in self.release_info["mingw"]["cmake"]["archs"]:
                ...
                elif self.release_info["mingw"]["cmake"]["shared-static"] == "build_shared_libs":
        ...
        for meta_destdir, file_globs in self.release_info["mingw"]["files"].items():
            ...
            assert isinstance(file_globs, list), f"'{file_globs}' in release_info.json must be a list of globs"
            for file_glob in file_globs:
                ...
                self.add_files_recursively(archived_files=archived_files, arcdirs=[f"{arc_root}{d}" for d in destdirs], paths=[self.root / p for p in relative_file_paths])
        print("... done")
    ...
    def create_android_archives(self, android_api: int, android_home: Path, android_ndk_home: Path) -> None:
        ...
        if "dependencies" in self.release_info["android"]:
            for dep, depinfo in self.release_info["android"]["dependencies"].items():
        ...
        with Archiver(zip_path=aar_path) as archiver:
            ...
            for module_name, module_info in self.release_info["android"]["modules"].items():
                ...
                if "includes" in module_info:
                    for arc_incl_rel_destdir, include_globs in module_info["includes"].items():
                        arc_incl_rel_destdir = self._configure_text(arc_incl_rel_destdir)
                        assert arc_incl_rel_destdir[:1] == "/" and arc_incl_rel_destdir[-1:] == "/", f"{arc_incl_rel_destdir} must start and end with slash '/'"
                        for include_glob in include_globs:
                            include_glob = self._configure_text(include_glob)
                            include_paths = glob.glob(include_glob, root_dir=self.root)
                            assert include_paths
                            for include in include_paths:
                                path = self.root / include
                                arcdir = f"{arcdir_prefab_include}{arc_incl_rel_destdir}".lstrip("/")
                                arc_name = path.name
                                archiver.add_file_path(arcpath=f"{arcdir}{path.name}", path=path)
                else:
                    archiver.add_file_data(arcpath=f"{arcdir_prefab_include}/.keep", data=b"\n", mode=0o100644, time=datetime.datetime.now())


            for arc_rel_destdir, file_rel_src_specs in self.release_info["android"].get("files", {}).items():
                arcdir_prefab_root = f""
                arc_rel_destdir = self._configure_text(arc_rel_destdir)
                assert arc_rel_destdir[:1] == "/" and arc_rel_destdir[-1:] == "/", f"{arc_rel_destdir} must start and end with slash '/'"
                for file_rel_src_spec in file_rel_src_specs:
                    if ":" in file_rel_src_spec:
                        file_relpath, arc_name = file_rel_src_spec.rsplit(":", 1)
                        path = self.root / file_relpath
                        arcdir = f"{arcdir_prefab_root}{arc_rel_destdir}".lstrip("/")
                        arcpath = f"{arcdir}{arc_name}"
                        if path.suffix == ".in":
                            data = self._configure_file(path)
                            ...
                            archiver.add_file_path(arcpath=f"{arcdir}{arc_name}", path=path)
                    else:
                        files = glob.glob(file_rel_src_spec, root_dir=self.root)
                        assert files
                        for file in files:
                            path = self.root / file
                            arcdir = f"{arcdir_prefab_root}{arc_rel_destdir}".lstrip("/")
    ...
    def _build_msvc_msbuild(self, arch_platform: VsArchPlatformConfig, vs: VisualStudio):
        ...
        built_paths = [
            self.root / arch_platform.configure(f) for msbuild_files in self.release_info["msvc"]["msbuild"]["files"] if not msbuild_files.get("prebuilt", False) for f in msbuild_files["paths"]
        ]
    ...
    def _build_msvc_cmake(self, arch_platform: VsArchPlatformConfig, dep_roots: list[Path]):
        ...
        built_paths = [
            install_path / arch_platform.configure(f) for msbuild_files in self.release_info["msvc"]["cmake"]["files"] for f in msbuild_files["paths"]
        ]
        ...
        with Archiver(zip_path=zip_path) as archiver:
            for msbuild_files in self.release_info["msvc"]["cmake"]["files"]:
                if "lib" in msbuild_files:
                    arcdir = arch_platform.configure(msbuild_files["lib"])
                    for p in msbuild_files["paths"]:
                        p = arch_platform.configure(p)
                        archiver.add_file_path(path=install_path / p, arcpath=f"{arcdir}/{Path(p).name}")
            for extra_files in self.release_info["msvc"]["files"]:
                if "lib" in extra_files:
                    arcdir = arch_platform.configure(extra_files["lib"])
                    for p in extra_files["paths"]:
    ...
    def _build_msvc_devel(self) -> None:
        ...
        with Archiver(zip_path=zip_path) as archiver:
            for extra_files in self.release_info["msvc"].get("msbuild", []).get("files", []):
                assert "devel" in extra_files
                for meta_glob_path in extra_files["paths"]:
                    assert "@" in meta_glob_path or "@" in extra_files["devel"]
                    for arch in self.release_info["msvc"]["msbuild"]["archs"]:
                        ...
                        root_path = self.root
                        paths = glob.glob(glob_path, root_dir=root_path)
                        assert paths, f"glob '{glob_path}' must expand to files"
            ...
            for extra_files in self.release_info["msvc"].get("cmake", []).get("files", []):
                assert "devel" in extra_files
                for meta_glob_path in extra_files["paths"]:
                    assert "@" in meta_glob_path or "@" in extra_files["devel"]
                    for arch in self.release_info["msvc"]["cmake"]["archs"]:
                        arch_platform = self._arch_to_vs_platform(arch=arch)
                        glob_path = arch_platform.configure(meta_glob_path)
                        root_path = self._arch_platform_to_install_path(arch_platform)
                        paths = glob.glob(glob_path, root_dir=root_path)
                        assert paths, f"glob '{glob_path}' must expand to files"
                        dst_subdirpath = arch_platform.configure(extra_files['devel'])
                        for path in paths:
            ...
            for extra_files in self.release_info["msvc"]["files"]:
                if "devel" in extra_files:
                    for meta_glob_path in extra_files["paths"]:
                        assert "@" not in meta_glob_path
                        assert "@" not in extra_files["devel"]
                        arch_platform = self._arch_to_vs_platform(arch=arch)
                        glob_path = arch_platform.configure(meta_glob_path)
